...
internal sealed class RowJsonWriter(Schema schema, IReadOnlyList<int> columns)
{
    ...
    // which this document is never rendered into; relaxed escaping keeps the JSON minimal
...
/// <summary>Scalar text formatting shared by JSON string values and document ids.</summary>
internal static class ArrowScalars
{
    public static string? Format(IArrowArray column, int row)
    {
        ...
        return column switch
        {
            ...
            // than throwing, so a 38-digit amount would land in the index silently truncated.
